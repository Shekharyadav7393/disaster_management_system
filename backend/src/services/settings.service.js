...
export const getPublicSettingsSnapshot = async () => {
  ...
  return {
    paymentAccount: {
      orgName: payment.orgName || process.env.ORG_NAME || "Disaster Management Support Fund",
      orgDescription: payment.orgDescription || process.env.ORG_DESCRIPTION || "Support verified disaster relief operations with money or essential supplies.",
      razorpayEnabled: Boolean((payment.razorpayEnabled && payment.razorpayKeyId) || process.env.RAZORPAY_KEY_ID),
      razorpayKeyId: payment.razorpayKeyId || process.env.RAZORPAY_KEY_ID,
      paymentLink: payment.paymentLink || process.env.PAYMENT_LINK,
      upiEnabled: payment.upiEnabled || Boolean(process.env.UPI_ID),
      upiId: payment.upiId || process.env.UPI_ID,
      upiName: payment.upiName || process.env.UPI_NAME || process.env.ORG_NAME,
      bankEnabled: payment.bankEnabled || Boolean(process.env.BANK_ACCOUNT_NUMBER),
      accountHolderName: payment.accountHolderName || process.env.BANK_ACCOUNT_NAME,
      bankName: payment.bankName || process.env.BANK_NAME,
      accountNumber: payment.accountNumber || process.env.BANK_ACCOUNT_NUMBER,
      ifscCode: payment.ifscCode || process.env.BANK_IFSC,
      accountType: payment.accountType || process.env.BANK_ACCOUNT_TYPE || "savings",
    },
